...
const allProducts=Object.values(media.products).flat();
const groups=categories.map((name,index)=>{const images=index===0?allProducts:media.products[name];return (images?.length?images:[null,null,null]).map((src,item)=>({src,category:index,item,count:images?.length||3,label:`${name} ${item+1} 이미지 영역`}))});
...
export function GoodsCarousel({categoryIndex,onCategoryChange}){
 ...
 <div className={`rolling-track ${step?'moving':''}`} style={{'--slide-step':step*progress}}>{[-2,-1,0,1,2].map(offset=><div key={offset} style={{'--focus':1-Math.min(1,Math.abs(offset-step*progress))}} className={`product-card ${offset===0?'current':''}`} aria-hidden={offset!==step}><MediaSlot src={slides[wrap(product+offset)].src} label={slides[wrap(product+offset)].label}/></div>)}</div>
